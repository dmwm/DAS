...
#        dquery._filters = []
#        self.assertEqual([], list(dquery.filters))
#        dquery._filters = filters
#        self.assertEqual(expect, list(dquery.filters))
...
#        addr = repr(dasquery).split()[-1].replace('>', '')
#        msg = "<DASQuery@%s: query='''%s''' instance=%s>" \
#                % (addr, iquery, dasquery.instance)
