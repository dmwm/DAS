...
\end{center}
\caption{\label{dasbench}Benchmark showing the average latency for fetching a single randomly-selected document from a wildcard query, with a given number of parallel clients. The cache contains $5\times10^7$ records. The red line shows the performance of the complete DAS stack, and the blue line the time spent by MongoDB.}
\end{figure}
