...
\textbf{\color{red}TODO: Overview}


\subsection{Keyword search over DBs}
The problem of Keyword search over Relational Databases (or also semi-structured sources like XML) has received a significant attention by the research community over the last decade. There exist many papers and a couple of quite comprehensive PhD. dissertations\cite{PhD_2011, PhD_2012} describing performance optimization details, etc. 

There was a number of variations of the problem including returning of only Top-k results expecting that users only care about the quality of limited number of results but not completeness of results, ... \textbf{\color{red}describe this in details}
...
Researchers from \textit{ETHZ} have been investigating Generation of SQL for Business users in collaboration with \textit{Credit Suisse}.  To converting natural language queries (that in addition to keyword search could convey some semantic structure) into SQL statements they employed some natural language processing and used meta-data describing the schema and the domain (ontologies at multiple levels). 

Even on a large data-warehouse of ~220GB data with a complex schema of 300 tables they reported fairly promising results that proves that if good meta-data is available, generating even quite complex SQL  (n-way joins with aggregations, etc) is quite feasible for computer making it 'much easier for business users to interactively explore highly-complex data warehouses' \cite[p.932]{ethz2012}. 

\subsubsection*{Automatic mapping between distributed services}
{\color{red}an interesting approach but not for CERN...}
...
\subsection{Searching Heterogeneous web services}
...
- if entered keyword query suggest a structured query, optionally with description in natural language
