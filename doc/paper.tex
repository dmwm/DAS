...
%Even though it would be interesting to apply technique 
%discussed in \cite{DBS-QL} to a federated DB it still requires 
%an access to the underlying database schema.
...
%The information retrieval were triggered on demand and stored into the cache.
...
%located in France, Germany, Italy, Spain, Taiwan, United Kingdom and United States
...
%This was achieved by using Dijkstra's shortest path algorithm 
...
%The DBS-QL keys exposed to end-users were chosen based on a jargon used by
%physicists. They're internally mapped into database entities and attributes, 
...
%For each GET request we performed the following action.
...
plug-ins and store results into the cache. Finally, the results were
retrieved from the cache and delivered to the user. 

The mapping between user queries, the DAS keys used in a query, 
and API names/parameters was stored in a Mapping DB. Such mapping was 
used by data-service plug-ins. For example
\begin{verbatim}
...
\end{verbatim}
represents the mapping between \verb+site+ DAS keyword and
\verb+CMSNametoSE+ API provided by SiteDB data-service (the data-service
URLs were kept in the configuration file).
The data-service plug-in implementation were similar and their logic was 
abstracted, while a few extentions were required, e.g. specific 
...
%the DAS cache server works as a proxy to this server and used host certificate 
...
%indexes (GB) & 135.9 & 57.2 & 12.6 & 0.004 & 2.7 \\
%\hline
%\hline
%\end{tabular}
%\caption{
...
% since the first one was injected into
