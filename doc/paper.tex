...
we were able to store DAS records of arbitrary structure, e.g.
...
\end{verbatim}
associated with a \verb+block+ key who had the following attributes
...
a nested structures were allowed, e.g. \verb+block.replica.size+. Our users 
where able to specify a context of their queries by using appropriate 
...
All input queries were transformed into MongoDB query syntax who is itself
...
were provided information about CMS blocks. In former case, DBS 
...
\end{verbatim}
The \verb+das_id+ part refers to api call records stored in Analytics DB,
