...
class DASAnalyticsController(DAS.core.das_robot.Robot):
    ...
    def load_modules(self,config):
        """
        Import all the modules defined in DASAnalyticsConfig.modules, then load the necessary classes.
        Set up the pipes for communication, and add any message subscriptions defined by services.
        ...
        for a_name,(a_classname,a_config) in config._analysers.items():
            ...
            if a_class:
                ...
                if validation==True:
                    ...
                    self.analysers[a_name] = a_class(a_name,a_config,self.das_config,slave_pipe)
                ...
                print "Class '%s' for analyser '%s' not found."%(a_classname,a_name)
                raise
        ...
        for s_name,(s_classname,s_config) in config._services.items():
            ...
            if s_class:
                ...
                print "Class '%s' for service '%s' not found."%(s_classname,s_name)
                raise
        
    def daemonize(self):
        """
        Daemonize using the method from das_robot only if daemon=True at __init__.
        """
        if self.daemon:
    ...
    def log(self,importance,message):
        """
        Log a message with given importance and text. Will only work if something has subscribed to log-type messages.
        """
        self.master_pipe.send({'type':'log','time':time.time(),'level':importance,'text':message, 'src':'core'})
    
    def run(self):
        """
        Actually run the analysis controller. This starts all the analysers and services, then
        goes into a loop handling any messages sent and checking that the analysers remain alive.
        """
        ...
        self.timers += [{'interval':self.config.heartbeat_interval,'func':self.check_heartbeat}]
        self.timers += [{'interval':10,'func':self.log,'args':(0,'still alive')}]
    ...
    def check_heartbeat(self):
        """
        Check that all our analysers and services are responding to 'ping' signals. Each ping
        has a sequence number, and are generally emitted at ~minute (configurable, 'heartbeat_interval')
        intervals. Emit a warning if the last received message is more than 'heartbeat_warn' sequence number out of date.
        Restart the relevant module if more than 'heartbeat_restart' out of date.
        If a module is currently doing work, then this takes control away from the main loop, which handles
        heartbeat, so some latitude needs to be given here if the analysis is CPU intensive.
        """
        for name, (seq,when) in self.last_heartbeat.items():
            if seq < self.current_heartbeat - self.config.heartbeat_warn:
                self.log(1,'expired heartbeat for %s (seq=%s, time=%s)'%(name,seq,when))
            if seq < self.current_heartbeat - self.config.heartbeat_restart:
                self.log(2,'restarting %s due to expired heartbeat (seq=%s, time=%s)'%(name,seq,when))
    ...
    def handle_msg(self,msg):
        """
        Handle incoming messages. Make sure the format is good (ie, a dictionary with a 'type' field).
        If so, check if any service has subscribed to this message type, then see if there is a function
        that handles messages of this type. This function uses a fixed list instead of introspection to avoid
        the possibility of badly-typed messages interfering with core operation.
        """
    ...
    def stop_child(self,name):
        """
        Try and gently stop a child. This emits the 'stop' signal, which should cause it to stop and
        finalise neatly if possible. A timer action is then scheduled for 'stop_time' to kill the child
        ...
        if name in self.analysers:
            ...
            return
        pipe.send({'type':'stop'})
        self.timers += [{'func':self.kill_child,'args':(name,),'next':time.time()+self.config.stop_time}]
        
        
    def kill_child(self,name):
        """
        Get rid of a child process, politeness be damned. If the process object still exists and is alive,
    ...
    def handle_msg_ping(self,msg):
        """
        Handle reply 'ping' messages, updating the sequence number in the last_heartbeat.
        """
    ...
    def handle_msg_broadcast(self,msg):
        ...
        for name,pipe in pipe_dict.items():
            if broadcast_class=='*' or broadcast_class=='' or fnmatch.fnmatch(name,broadcast_match):
                pipe.send(payload)
    
    def handle_msg_stop(self,msg):
        """
        Try and shut down gracefully, if possible. Emit the stop signal to each child,
        then join each process for up to 'stop_time'. After this method returns, the main loop
        should terminate.
        """
        self._stop = True
    ...
    def handle_msg_kill(self,msg):
        """
        Shut down ungracefully. Send SIGTERM to all children, then exit hard.
        """
        for name, process in self.analysers.items():
            process.terminate()
        for name, process in self.services.items():
    ...
    def handle_subscriptions(self,msg):
        """
        Match messages to subscription rules, using fnmatch (shell-type wildcard matching).
        This might need re-ordering for efficiency.
        """
        src = msg.get('src','')
        msgtype = msg.get('type','')
        for (target,match_class,match_src,match_msg) in self.subscriptions:
            if match_class=='all' or (match_class=='analyser' and src in self.analysers) or (match_class=='service' and src in self.services):
                if match_src=='' or match_src=='*' or fnmatch.fnmatch(src,match_src):
                    if match_msg=='' or match_msg=='*' or fnmatch.fnmatch(msgtype,match_msg):
                        msg['subscribed']=True
...
if __name__=='__main__':
    ...
    parser.add_option('--start',dest='start',action='store_true',help='Start Analytics Daemon',default=False)
    parser.add_option('--stop',dest='stop',action='store_true',help='Stop Analytics Daemon',default=False)
    parser.add_option('--restart',dest='restart',action='store_true',help='Restart Analytics Daemon',default=False)
    parser.add_option('--status',dest='status',action='store_true',help='Status of Analytics Daemon',default=False)
    parser.add_option('-f','--foreground',dest='daemon',action='store_false',help='Ward against demonic intervention',default=True)
    parser.add_option('-d','--das_config',dest='das_config',action='store',help='Location of DAS config file',default=None)
    ...
    if not os.path.exists(analytics_config):
        print "Analytics Config (%s) doesn't appear to exist."%analytics_config
        raise
    ...
    if opts.das_config == None:
        ...
        if os.path.exists(opts.das_config):
            das_config = DAS.utils.das_config.das_readconfig(opts.das_config)
        else:
            print "DAS Config (%s) doesn't appear to exist."%opts.das_config
            raise
