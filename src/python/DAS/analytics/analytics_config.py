        ...
        if isinstance(value, self.type):
            if self.choices:
                if value in self.choices:
                    ...
                    desc = 'Bad value for option "%s" (found "%s" valid %s).' % (
                                                                self.name,
                                                                value,
                                                                self.choices)
                    return desc
            ...
            desc = 'Bad type for option "%s" (found "%s" expected "%s").' % (
                                                                self.name,
                                                                type(value),
                                                                self.type)
            return desc
...
class DASAnalyticsConfig(object):
    ...
    def __init__(self):
        self._options = {}
        self._tasks = []
        #self.logger = logging.getLogger('DASAnalytics.Config') #not a lot of use before we've init'd loggers
    ...
    def add_task(self, name, classname, interval, 
        ...
        if task in self._tasks:
            #self.logger.warning('Task with this name, class, interval already exists.')
            pass
        else:
    ...
    def name_to_longcmd(self, name):
        "Convert an option name into a --option."
            ...
            for option in self.options.values():
                if option.group == grp and not option.confonly:
                    ...
                    if option.type and option.type in self.type_map:
                        ...
                        kwargs['action'] = 'store_false' if option.default else 'store_true'
                    if option.help:
